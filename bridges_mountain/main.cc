#include "Bridges.h"
#include "ColorGrid.h"
#include "tile.h"
#include <iostream>
#include <fstream>
#include <sstream>
#include <unordered_set> 
#include <queue> 
#include <cmath> 
#include <vector> 
#include <string>
#include <algorithm>
//#include <utility>
#include <cassert>
using namespace std;
using namespace bridges;

//#define INF 999 

// This program implements the mountain paths assignment in C++. See the README.md for a 
// detailed description

// function prototypes
ColorGrid *getImage (int *img_arr, int imgWidth, int imgHeight, int maxVal);
int *readData (int *imgWidth, int *imgHeight, int *maxVal, string file_name);
void findPath(int *img_arr, int imgWidth, int imgHeight, int startRow, int startCol, int endRow, int endCol, int maxVal);

int main(int argc, char **argv) {
	const int bridges_project_id = 1001;
	const string bridges_username = "RUND0WN";
	//const string bridges_api_key  = "";

	//Delete this line once you get registered on BRIDGES
	//static_assert(bridges_username != "username","You must register on BRIDGES and get your own usernsme and API key.");

	//bridges object initialization
	Bridges bridges(bridges_project_id, bridges_username, bridges_api_key);

	// read input elevation data
	int width, height, maxVal;
	int *elev_data;

	// parse command line arguments
	// argument 2 is input file
	string filename = "./colorado2.dat";
	if (argc >= 2) filename = argv[1]; //This is how you read a command line parameter
	elev_data = readData(&width, &height, &maxVal, filename);

	// set title
	//YOU: Change this to "Mountain Paths - Dijkstra's" - DONE
	//bridges.setTitle("Mountain Paths "s + filename + " - Greedy Algorithms Example");
	bridges.setTitle("Mountain Paths "s + filename + " - Dijkstra's");
	

	//WRK - Added this section
	//Get starting row and column - the greedy algorithm only needs starting row,
	// but you will use the two locations for doing Dijkstra's
	int startRow = 0, startCol = 0, endRow = 0, endCol = 0;
	cout << "Please enter starting row and col:\n"; //Only starting row is used for greedy mode
	cin >> startRow >> startCol;
	cout << "Please enter ending row and col:\n"; //Neither of these is used in greedy mode
	cin >> endRow >> endCol;

	//YOU: Replace the greedy algorithm with Dijkstra's or A* between the starting and ending points
	//YOU: Look at how findPath does it's think in regards to converting between a 2D location and a 1D array, as well as how it marks the final path by changing the elevation to -1 along the path
	// find path by applying a greedy algorithm from the left side of the screen to the right
	//findPath(elev_data, width, height, startRow, startCol, endRow, endCol, maxVal);
	findPath(elev_data, width, height, startRow, startCol, endRow, endCol, maxVal); //HEAP OVERFLOW
	// get the path written into a color grid for visualization
	//YOU: Change the path color to red
	ColorGrid *cg = getImage(elev_data, width, height, maxVal);

	// visualize
	bridges.setDataStructure(cg);
	bridges.visualize();

	delete cg; //WRK Fix memory leak in BRIDGES
	delete[] elev_data; //WRK Ditto
}

int *readData(int *width, int *height, int *maxVal, string file_name) {

	ifstream infile(file_name.c_str());

	// read header 
	int max = 0, w, h, val;
	infile >> w >> h;

	*width = w; *height = h;

	// allocate space for the input data
	// we will use a 1D array
	int size = w*h;
	int *elev_data = new int[size]; //HEAP OVERFLOW - RELOCATION OF MEMORY ADDRESS? 
	assert(elev_data); //WRK
	//free(elev_data);

	// read the elevation data
	for (int k = 0; k < size; k++) {
		infile >>  val;

		if (val == -1) val = -2;

		// update the max val
		max = (val > max) ? val : max;
	
		// store  in elevation array
		elev_data[k] = val;
	}
	// record the max value
	*maxVal = max;

	return elev_data;
}

// takes in the processed elevation data and returns a color grid for 
// visualization using BRIDGES
ColorGrid *getImage(int *elev_data, int width, int height, int maxVal) {
	// create color grid
	ColorGrid *cg = new ColorGrid(height, width);

	float pixel_val;
	int n = 0, gray;

	// load the elevation data
	for (int j = 0; j < height; j++)
	for (int k = 0; k < width; k++) {
		pixel_val = (float) elev_data[n++];
		// this is the path drawn in red across the image
		//cg->set(j, k, Color(255, 0, 255));
		if (pixel_val == -1.) cg->set(j, k, Color(255, 0, 0));
		else if (pixel_val < -1) cg->set(j,k, Color(0,0,128));
		else {
			// scale value to be within 0-255, for r,g,b range
			gray = (int) (pixel_val*255./maxVal);	
			cg->set(j, k, Color(gray, gray, gray));
		}
	}
	return cg;
}
/*
	int start = 0;
	while (make_processed.size() != 0) {
	}
*/
/*
void init() {
	int vertices = 0;
	for (size_t i = 0; i < vertices; i++) 
}
*/
/*
struct Pixel {
	int x,y;
	Pixel(int new_x, int new_y) {
		x = new_x;
		y = new_y;
	}
	vector<Pixel> vec;
};
*/
//A function prototype calculating the cost of moving from one tile to another
/*
int cost (int horiz, int vert) {
	const int lateral = 1840, diagonal = 2040	
	return cost;
}
*/
	//to_process.emplace(j,i,0,0,elev_data[i*imgWidth+j]); //Getting the elevation at some point in x,y form.  
	//to_process.emplace(j,i,0,0,sqrt( pow(startRow - i, 2) + pow(startCol - j, 2)));  
		//if (imgWidth < 0 && imgHeight < 0) //If elevation is negative, treat as blocking terrain 	
				//Tile temp = *make_processed.find(child); //Dereferences by copy   
				/*
				unordered_set<Tile>::iterator found = make_processed.find(child);
				if (found->distance > new_weight) {   
						found->distance = new_weight;
						found->from_x = processing.x; //Coming from the thing to be processed
						found->from_y = processing.y;
				}
					*/
		/*
		from = elev_data[processing.y * imgWidth + processing.x];
		to = elev_data[j * imgWidth + i];
		double new_weight = processing.distance + scale + 20 * abs(from - to); 
		*/

// determines the least effort path through the mountain starting a point on
// the left edge of the image
void findPath(int *elev_data, int imgWidth, int imgHeight, int startRow, int startCol, int endRow, int endCol, int maxVal) {
	unordered_set<Tile> make_processed; //Hash table. Can change the values of the children in a map instead of a set.  
	priority_queue<Tile, vector<Tile>, Tile_Comp> to_process; //Heap
	//Filling up the to_process list. 
	if (startRow < 0 || startRow >= imgHeight) {
		cout << "Invalid starting row.\n"; //WRK Let them know
		return;
	}
	Tile parent_tile(startCol, startRow, -1, -1, 0); //Stores parent tile
	to_process.emplace(parent_tile);  
	while (to_process.size() != 0) {
		//cout << "Inside the 1st while loop\n";
		Tile processing = to_process.top(); //Keeping track of the parent node	
		to_process.pop(); 
		if (make_processed.count(processing) || elev_data[processing.y * imgWidth + processing.x] < 0)  continue; //If the data is already processed, skip
		make_processed.insert(processing); //Adding parent node to the processing list
		if (processing.x == endCol && processing.y == endRow) break;
		//Adding children to the processing list
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				int scale = 0;
				if (i == 0 && j == 0 || (processing.x + i < 0 || processing.x + i == imgWidth || processing.y + j < 0 || processing.y + j == imgHeight)) continue;
				if (processing.x + i != processing.x && processing.y + j != processing.y) scale = 2601;
				else scale = 1840;
		//double new_weight = processing.distance + scale + 20 * abs(elev_data[processing.y * imgWidth + i + processing.x + j] - elev_data[processing.y * imgWidth + processing.x]); 
		double new_weight = processing.distance + scale + 20 * abs(elev_data[processing.y * imgWidth + processing.x] - elev_data[(processing.y + j) * imgWidth + processing.x + i]); 
				Tile child(processing.x + i, processing.y + j, processing.x, processing.y, new_weight);
				//if (make_processed.count(child) == 0) 
				to_process.push(child); 
			}
		}
	}
	//Take an element and do a trace back starting at the from node to the start node. 
	Tile from_node = *make_processed.find(Tile(endCol, endRow, 0, 0, 0)); //Starting at the from node
	elev_data[from_node.y * imgWidth + from_node.x] = -1;
	//Tracing back to the start node
	while (!(from_node == parent_tile)) {
		from_node = *make_processed.find(Tile(from_node.from_x, from_node.from_y, 0, 0, 0));
		cout << from_node.x << " " << from_node.y << endl;
		elev_data[from_node.y * imgWidth + from_node.x] = -1;	
	}
}
/*
	int col = 0, row = startRow; 
	int pix_addr = startRow*imgWidth + col;

	while (col < imgWidth-2) {
		// hold neighbors in array
		int neighb[3] = {0, 0, 0};

		// compute 2D address of pixel from 1D address
		row = pix_addr/imgWidth;
		col = pix_addr - row*imgWidth;
		int top_right = pix_addr - imgWidth + 1;
		int right = pix_addr + 1;
		int bottom_right = pix_addr + imgWidth + 1;
		
		int top = pix_addr - imgWidth + 1;
		int top_left = pix_addr + 1;
		int bottom_left = pix_addr + imgWidth + 1;
		
		int cnt;

		// color the pixel, its in the path
		elev_data[pix_addr] = -1; 

		// deal with boundary cases first
		if (row == 0) { 
			// must  go right or down
			cnt = 2;
			neighb[0] = right; neighb[1] = bottom_right;
		}
		else if (row == imgHeight-1){ 
			// must go right or up
			cnt = 2;
			neighb[0] = right; neighb[1] = top_right;
		}
		else { 	
			// general case
			// check the 3 right neighbors, make greedy decision 
			cnt = 3;
			neighb[0] = top_right; neighb[1] = right; neighb[2] = bottom_right;
		}

		// compute differences with right neighbors
		int min = maxVal;
		int diff[3];
		for (int k = 0; k < cnt; k++)  {
			int d = elev_data[pix_addr] - elev_data[neighb[k]];
			diff[k] = (d < 0) ? -d : d;
		}
		// choose pixe with the smallest difference
		switch (cnt) {
			case 2: 
				pix_addr =  (diff[0] <= diff[1]) ? neighb[0] : neighb[1];
				break;
			case 3:
				if (diff[0] < diff[1] && diff[0] < diff[2])
					pix_addr = neighb[0];
				else 
					if (diff[1] < diff[0] && diff[1] < diff[2])
						pix_addr = neighb[1];
				else 
					if (diff[2] < diff[0] && diff[2] < diff[1])
						pix_addr = neighb[2];
				else if ((diff[0] == diff[1]) || (diff[1] == diff[2]))
						pix_addr = neighb[1];
				else { 
					double val = drand48();
					pix_addr = (val < 0.5) ? neighb[0] : neighb[2];
			}
		}
		col++;
	}
*/
